...
TAGS:
  ...
  ;NEW PRICING RULES
    if (asinbool && !loneasinbool && marketDiff <= acceptDiff && asinRanking > asinHighRank && realAma > realFloor){
      price := setAma
      ambool := 1
    } else if ((asinbool && loneasinbool && asinRanking < asinHighRank) || (asinbool && realAma > realEbay && marketDiff > acceptDiff) ){
      price := setAma
      epbool := 1
    }else if (asinbool && realEbay > realAma && asinRanking > asinMidRank && ){
      price := setEbay
      apbool := 1
    } else if ((!asinbool) || ()){
      price := setEbay
